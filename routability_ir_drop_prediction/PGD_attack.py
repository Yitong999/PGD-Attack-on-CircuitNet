    ...
    while iter_num < arg_dict['max_iters']:
        ...
        if iter_num % save_freq == 0:
            ...
            with torch.no_grad():
                ...
                plt.imsave(f'/scratch/yc7900/eg/code/circuit_learning/CircuitNet/routability_ir_drop_prediction/imgs/{iter_num}_PGDadversarial_image.png', adv_img)
                pred_img = adv_prediction[0,0].detach().cpu().numpy()
                pred_img = (pred_img - pred_img.min()) / (pred_img.max() - pred_img.min())
                plt.imsave(f'/scratch/yc7900/eg/code/circuit_learning/CircuitNet/routability_ir_drop_prediction/imgs/{iter_num}_PGDadv_prediction.png', pred_img, cmap='gray')
                input_img = input[0]  # shape: [3, 256, 256]
                input_img = input_img.cpu().numpy()
                input_img = np.transpose(input_img, (1,2,0))
                input_img = (input_img - input_img.min()) / (input_img.max() - input_img.min())

                plt.imsave(f'/scratch/yc7900/eg/code/circuit_learning/CircuitNet/routability_ir_drop_prediction/imgs/{iter_num}_PGDinput_image.png', input_img)

                target_img = target[0,0].cpu().numpy()  # shape: [256, 256]
                target_img = (target_img - target_img.min()) / (target_img.max() - target_img.min())

                plt.imsave(f'/scratch/yc7900/eg/code/circuit_learning/CircuitNet/routability_ir_drop_prediction/imgs/{iter_num}_PGDtarget_image.png', target_img, cmap='gray')
                pred_img = prediction[0,0].detach().cpu().numpy()  # shape: [256, 256]
                pred_img = (pred_img - pred_img.min()) / (pred_img.max() - pred_img.min())
                plt.imsave(f'/scratch/yc7900/eg/code/circuit_learning/CircuitNet/routability_ir_drop_prediction/imgs/{iter_num}_PGDprediction_image.png', pred_img, cmap='gray')
            print('Images saved')
        epoch_loss = 0
